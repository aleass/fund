...
func (f *FundEaringsRank) GetData() {
	...
	common.FuncDb.Select("id,code").Model(&model.DfFundEarnings{}).Find(&dfEarningsMode)
	...
	common.FuncDb.Model(&model.DfFundList{}).Where("`type` LIKE '债券型%' and code not in (SELECT code FROM df_fund_earnings_rank GROUP BY code)").Find(&df)
...
func (f *FundEaringsRank) getEaringsRankUrlData(codes chan [2]string, closeChan chan struct{}, earningsMap *map[string]int64) {
	...
	for data := range codes {
		...
		common.FuncDb.Select("date").Model(&model.DfFundEarningsRank{}).Where("code = ?", code).
			Order("date desc").Limit(1).Find(&earningsRankMode)
		lastDate := earningsRankMode.Date

		defVal.CreatedAt = &now
		defVal.AddTime = now.Unix()
		...
		if hasEarnings {
			...
			if err := common.FuncDb.Exec(sql).Error; err != nil {
	...
	earings:
		...
		if len(list) > 0 {
			...
			if err := common.FuncDb.CreateInBatches(temp, 50).Error; err != nil {
				common.Logger.Error(err.Error())
			}
